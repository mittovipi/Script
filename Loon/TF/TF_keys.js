...
if (reg1.test($request.url)) {
    ...
    if ($persistentStore.read('request_id') !== null) {
      $notification.post('Thu thập thông tin TF', 'Thông tin lấy thành công, vui lòng đóng script lại!','')

    } else {
      $notification.post('Thu thập thông tin TF','Việc thu thập thông tin không thành công, vui lòng bật công tắc Mitm qua HTTP2 và khởi động lại VPN và Ứng dụng TestFlight!','')
    }
    $done({})
}
if (reg2.test($request.url)) {
  ...
  $notification.post("Tự động tham gia TestFlight", `thêm APP_ID: ${id}`, `hiện hành ID: ${appId}`);
  $done({})
}
function unique(arr) {
